...
def cstmr(cid):
    ...
    if request.method == 'POST':
        ...
        try:
            query(
                'UPDATE customer SET customer_name=%s, phone_number=%s, password=%s WHERE customerID=%s',
                (name, phone, password, cid), commit=True
            )
    ...
    try:
        with conn.cursor() as cur:
            ...
            cur.execute(
                ...
                (session.get('technician_id'), aid, aid)
            )
            cur.execute(
...
def product_add():
    try:
        query('INSERT INTO product (product_name, stock_quantity, product_type) VALUES (%s,%s,%s)',(request.form['product_name'].strip(), request.form['stock_quantity'].strip(), request.form['product_type'].strip()), commit=True)
        flash('Product "' + request.form['product_name'].strip() + '" added.', 'success')
    except Exception as e:
        flash(f'Error: {e}', 'danger')
    return redirect(url_for('products'))
